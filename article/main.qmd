...
O objetivo deste trabalho é, portanto, preencher essa lacuna da literatura econômica regional e analisar o comportamento da atividade bancária no espaço urbano, sob o prisma pós-keynesiano, realizando testes empíricos para verificar localização, distribuição, qualidade e diversidade do crédito no município de São Paulo, no ano de 2010. Com isso, desdobram-se os seguintes objetivos específicos:

-   Georreferenciar os dados das agências bancárias (ESTBAN);

-   Agregar os dados por setor censitário;

-   Cruzar com dados dos CENSO 2010, agregados por setor censitário;

-   Agregar por distritos;
...
ggsave("irc.pdf", plot = irc_cow, width = 5000, height = 5000, units = "px", dpi = 320)
```

\fig{Índice Regional do Crédito (IRC) no município de São Paulo (2010).}{fig:irc}{irc.pdf}{Elaboração própria.}
...
ggsave("ire.pdf", plot = ire_cow, width = 5000, height = 5000, units = "px", dpi = 320)
```

\fig{Índice Regional de Empréstimos (IRE) no município de São Paulo (2010).}{fig:ire}{ire.pdf}{Elaboração própria.}
...
ggsave("irf.pdf", plot = irf_cow, width = 5000, height = 5000, units = "px", dpi = 320)
```

\fig{Índice Regional de Financiamentos (IRF) no município de São Paulo (2010).}{fig:irf}{irf.pdf}{Elaboração própria.}
...
ggsave("ird.pdf", plot = ird_cow, width = 5000, height = 5000, units = "px", dpi = 320)
```

\fig{Índice Regional de Depósitos (IRD) no município de São Paulo (2010).}{fig:ird}{ird.pdf}{Elaboração própria.}
...
ggsave("irl.pdf", plot = irl_cow, width = 5000, height = 5000, units = "px", dpi = 320)
```
...
ggsave("irr.pdf", plot = irr_cow, width = 5000, height = 5000, units = "px", dpi = 320)
```

\fig{Índice Regional de Risco (IRR) no município de São Paulo (2010).}{fig:irr}{irr.pdf}{Elaboração própria.}

O Índice Regional de Risco da figura \ref{fig:irr} nos dá indício de como os bancos enxergam centro e periferia. Mesmo numa grande metrópole como São Paulo, com regiões igualmente populosas, é possível observar a dinâmica de centro e periferia na atuação dos bancos. As regiões periféricas no entorno do centro têm agências que provisionam muito mais crédito em relação à renda da região para cobrir possíveis inadimplências. Essa percepção de risco dos bancos pode ser determinante na oferta de crédito. Os bancos estão mais propensos a conceder crédito para clientes em regiões consideradas de menor risco, como o centro da cidade. Isso ocorre porque eles acreditam que esses clientes têm maior probabilidade de pagar suas dívidas. Essa dinâmica pode ter um impacto negativo no desenvolvimento econômico das regiões periféricas. A falta de acesso ao crédito pode dificultar a geração de renda e a criação de empregos nessas regiões.

\fig{Índice Regional de Lucros (IRL) no município de São Paulo (2010).}{fig:irl}{irl.pdf}{Elaboração própria.}
...
ggsave("plp.pdf", plot = plp_cow, width = 5000, height = 5000, units = "px", dpi = 320)
```

\fig{Preferência por liquidez do público (PLP) no município de São Paulo (2010).}{fig:plp}{plp.pdf}{Elaboração própria.}

Como era esperado pela literatura [@dow82;@chick88;@crocco2006], as regiões periféricas apresentam maior preferência por liquidez. Isso pode ser explicado por alguns fatores, como:
...
ggsave("plb.pdf", plot = plb_cow, width = 5000, height = 5000, units = "px", dpi = 320)
```

\fig{Preferência por liquidez dos bancos (PLB) no município de São Paulo (2010).}{fig:plb}{plb.pdf}{Elaboração própria.}
...
ggsave("plbe.pdf", plot = plbe_cow, width = 5700, height = 5000, units = "px", dpi = 320)
```

\fig{Preferência por liquidez dos bancos - endógena (PLBe) no município de São Paulo (2010).}{fig:plbe}{plbe.pdf}{Elaboração própria.}
...
ggsave("ico.pdf", plot = ico_cow, width = 5200, height = 5000, units = "px", dpi = 320)
```

\fig{Índice de Concentração de Operações (ICO) no município de São Paulo (2010).}{fig:ico}{ico.pdf}{Elaboração própria.}
...
ggsave("icb.pdf", plot = icb_cow, width = 5200, height = 5000, units = "px", dpi = 320)
```

\fig{Índice de Concentração Bancária (ICB) no município de São Paulo (2010).}{fig:icb}{icb.pdf}{Elaboração própria.}
...
Com auxílio do critério de Kaiser e do modelo Broken-Stick (@fig-acp), foram escolhidos 2 componentes para serem analisados, pois já concentram 66% da variância total e condensam múltiplas dimensões para uma análise bidimensional.
...
#| fig-scap: "Fonte: Elaboração própria"
...
ggsave("icf.pdf", plot = icf_cow, width = 5000, height = 5000, units = "px", dpi = 320)
```

\fig{Índice de Centralidade Financeira (ICF) no município de São Paulo (2010).}{fig:icf}{icf.pdf}{Elaboração própria.}
...
ggsave("iqf.pdf", plot = iqf_cow, width = 5000, height = 5000, units = "px", dpi = 320)
```

\fig{Índice de Qualidade Financeira (IQF) no município de São Paulo (2010).}{fig:iqf}{iqf.pdf}{Elaboração própria.}
...
As figuras \ref{fig:moran} e \ref{fig:moranicf} mostram a relação entre $y$ e $Wy$ (*lagged*), sendo $y =$ IQF ou ICF e $W =$ matriz de pesos de 5 (IQF) e 7 (ICF) vizinhos mais próximos. Ambos índices de Moran foram estatisticamente diferentes de zero, o que significa que há autocorrelação espacial para IQF e ICF, no entanto, a autocorrelação espacial do IQF é muito mais forte, indicando que o espaço importa mais na difusão de qualidade e diversidade de crédito, do que na questão de centralidade e volume de crédito. Como o bairro Sé é um *outlier* na centralidade do crédito, é possível que ele esteja mascarando algum aspecto das demais centralidades de crédito.
...
ggsave("imoraniqf.pdf", plot = i_moran_iqf, width = 40, height = 20, units = "cm", dpi = 320)
```

\fig{I de Moran para o IQF}{fig:moran}{imoraniqf.pdf}{Elaboração própria.}

```{r, echo=FALSE, message=FALSE, warning=FALSE, error=FALSE, eval=FALSE}
#| label: fig-moran-icf
#| fig-cap: "I de Moran para o ICF."

moran_i_icf <- spdep::moran.test(shape$comp1, knn7_lista)
moran_local_icf <- spdep::localmoran(shape$comp1, knn7_lista, alternative = "two.sided")

shape$ylag_icf <- lag.listw(knn7_lista, shape$comp1)

shape$quadrante_icf <- ifelse(shape$p_value <= 0.05,
  ifelse(
...
ggsave("imoranicf.pdf", plot = i_moran_icf, width = 40, height = 20, units = "cm", dpi = 320)
```

\fig{I de Moran para o ICF}{fig:moranicf}{imoranicf.pdf}{Elaboração própria.}
...
moranlocal_map <- ggplot2::ggplot() +
  ...
  ggrepel::geom_text_repel(
    data = subset(shape, shape$quadrante %in% c("High-High", "High-Low", "Low-Low")),
...
ggsave("moranlocal.pdf", plot = moranlocal_map, width = 5000, height = 5000, units = "px", dpi = 320)
```

\fig{Clusters LISA (I de Moran Local) - IQF}{fig:moranlocal}{moranlocal.pdf}{Elaboração própria.}
...
ggsave("moranlocalicf.pdf", plot = moranlocalicf_map, width = 5000, height = 5000, units = "px", dpi = 320)
```

\fig{Clusters LISA (I de Moran Local) - ICF}{fig:moranlocalicf}{moranlocalicf.pdf}{Elaboração própria.}
...
Pode-se observar uma formação de cluster *Low-Low* na Zona Leste (extremo leste) de São Paulo para ambos os índices, indicando que se trata de uma periferia em termos de crédito, aglomerando distritos que compartilham dos mesmos baixos índices.

Além disso, observa-se que o distrito Jardim Paulista se encontra em clusters *High-High* em ambos os índices, o que demonstra uma certa influência sobre os seus vizinhos, positiva para o IQF e negativa para o ICF, ou seja, compartilha de qualidade e diversidade de crédito com os distritos próximos e compete em termos de operações e centralidade de crédito. Surpreende o distrito Sé, com alto ICF não ter formado um cluster *High-High* ou *High-Low*.
...
reg_slx <- spatialreg::lmSLX(formula = "rpc ~ ICF + IQF", data=spatial, listw=knn4_lista)
...
lr_test_print |>
  stargazer::stargazer(type="latex", summary = FALSE, notes = c(paste("LR: ", lr_test$statistic), paste("p.value: ", lr_test$p.value)))
```

Como o teste LR disse que os modelos são estatísticamente diferente e o que tem melhor ajuste é o SLX, opta-se por escolhê-lo neste trabalho. A @tbl-slx mostra seu resultado: 
...
ggplot2::ggplot(reg_slx) +
  ggplot2::geom_point(mapping = aes(x = IQF, y = y)) +
  ggplot2::geom_abline(slope = as.numeric(reg_slx$coefficients[4]), intercept = as.numeric(reg_slx$coefficients[1]))
```
\newpage
